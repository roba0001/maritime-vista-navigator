...
const MaritimeMap: React.FC<MaritimeMapProps> = ({ selectedRole, onBack, onBackToStart }) => {
  ...
  const addRealisticShippingRoutes = (globe: any) => {
    ...
    const oceanRoutes = [
      ...
      {
        from: "strait_of_hormuz",
        to: "strait_of_malacca",
        waypoints: [
          [26.5669, 56.2788],
          [20.0, 65.0],
          [10.0, 75.0],
        ...
        <div className="absolute top-4 left-4 z-10 flex space-x-3">
          ...
          <Card className="bg-white/90 backdrop-blur-sm">
            <CardContent className="p-4 flex items-center space-x-3">
              <div
                className={`px-3 py-1 rounded-full text-white text-sm ${getRoleColor(
                  selectedRole
                )}`}
              >
